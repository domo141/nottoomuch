...
# Last modified: Sun 19 Oct 2014 12:03:52 +0300 too
...
# ... but that just split too eagerly (saw >= 2 headers (w/o From ) - new mail)

# XXX this is a bit naive, 'From ' w/o some following headers is also accepted
# XXX but it would be considerable more complex to support caching of lines...
perl -e 'my $el = 1; while (<STDIN>) { if ($el && s/^From /X-From-Line: /) {
		open P, "|-", @ARGV or die "Executing @ARGV: $!\n";
	}
	$el = (/^$/)? 1: 0;
	print P $_;
} close P' "$@" < "${tmmf:-$mbox}"
